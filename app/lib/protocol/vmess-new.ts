...
const VMessV2ToV1Link = (obj: VMessV2): string => {
  ...
  Object.keys(others).forEach((k) => {
    ...
    const newValue = v1Converter[newKey]
      ? v1Converter[newKey](others[k])
      : others[k];
...
export class VMess extends Protocol {
  constructor(link: string) {
    ...
    if (
      /^vmess:\/\//i.test(link) &&
      (this.isVMessLinkV1(link) || this.isVMessLinkV2(link))
    ) {
  ...
  private isVMessLinkV2(link: string): boolean {
    const linkBody = link.replace(/^vmess:\/\//i, '');
    return (
      !linkBody.includes('?') && this.tryToParseJson(this.tryDecode(linkBody))
    );
  }
  ...
  private parseV2Link(link: string): VMessV2 {
    ...
    Object.entries(obj).forEach(([key, value]) => {
      ...
      params[newKey] = v2ToStdV2Converter[newKey]
        ? v2ToStdV2Converter[newKey](value)
        : value;
    });
    return params;
  }
  ...
  genOutboundFromLink() {
    const {
      ...
      sni,
      fp,
    } = this.shareLinkParseData as VMessV2;
    const streamType: 'tcp' | 'kcp' | 'ws' | 'h2' | 'quic' | 'grpc' =
      net ?? 'tcp';
    this.protocol = 'vmess';
    // @ts-ignore configuration is complete in a easy way
    this.streamSettings[
      `${streamType === 'h2' ? 'http' : streamType}Settings`
    ] =
      this.streamSettingsTemplate[
        `${streamType === 'h2' ? 'http' : streamType}Settings`
      ];
